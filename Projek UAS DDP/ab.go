...
func tambahResep(resepMakanan *map[string]Resep, fileName string) {
	...
	for {
		...
		scanner.Scan()
		bahanResep = scanner.Text()

		if bahanResep == "_" {
			break
		}

		fmt.Printf("Satuan bahan ke - %d: ", bahanIndex) // Meminta input satuan bahan
		var satuanBahan string
		scanner.Scan()
		satuanBahan = scanner.Text()

		bahan = append(bahan, fmt.Sprintf("%d. %s - %s", bahanIndex, bahanResep, satuanBahan)) // Menambahkan satuan bahan
		bahanIndex++
	}
...
AYm
1. oreng mail - ayam dd efef, 2. mein - nenk, 3. nfiekn - nnje, 4. ndn - dle
ayajd fd, hd, sdnis d,  s djsb ahk

Mie Goreng
Mie instan, Telur, Kecap, Bawang goreng
Rebus mie hingga matang, tiriskan., Kocok telur dan tumis hingga setengah matang., Masukkan mie ke dalam telur, tambahkan kecap, aduk hingga merata., Sajikan dengan taburan bawang goreng.
